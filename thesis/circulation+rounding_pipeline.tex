...
So an outside vertex $i$ is mostly like a chord on backbone cycle with activation probability.
...
to move from one vertex to another. Since we do not care about log factor,
we can use each backbone edge poly-log times, but it can happen that poly-log is 
not enough in this naive adaptive policy. In circulation lemma, we also paid for the chords
on backbone tour, i.e. those $u \rightarrow i \rightarrow v$ chords where $i$ is the outside vertex.
Since ciculation lemma gives bounded cost, we can also use these edges poly-log times.
But notice that, each such arc has non-integral weight in circulation. So when we say poly-log times,
we are actually saying we can use it no more than poly-log multiplied by the flow weight $x_e$.
Or in another way, since after rounding each edge has flow weight either $0$ or $p_i$,
we can assume we can use the chord when the corresponding outside vertex is active, bacause
