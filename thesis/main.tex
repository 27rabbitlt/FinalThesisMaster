...
on~$\varepsilon$.
\manuel{Perhaps explicitly say that $c_\varepsilon$ is a constant.}
...
The goal is to find a tour that visits all active vertices and minimizes the expected travel cost, where the cost comes from the underlying metric on \manuel{the?} vertices.
...
,,,,,,,,,,,,
\sharat{These paras needs rephrasing. You jumped straight to the stochastic model. Go a bit slower. The first para can talk broadly about the importance of TSP, routing, scheduling etc. and also how uncertainty is present in real world optimization problems. Next para can talk about classical TSP. Describe the setup and maybe move \Cref{sec:metric-spaces-tours} here. Mention that the problem is APX hard and so constant factor approximation algorithms have been intensively studied (cite couple of papers). Then mention that the problem can be solved in exponential time using DP. Now start a new para by saying that we will consider the stochastic generalization where ... It's a good idea to introduce the $Q(A)$ notation. Make a remark that when $p_v = 1$ for all vertices, we recover the classic setting, so StochTSP is at least as hard as the deterministic version. New para again. Now talk about the a posteriori setting. This is the simplest of all because it's just the expected value of classical TSP solutions. Next, mention the a priori model in a new para. This is also a natural model because you are still computing one master tour (like in classical TSP) and the cost is some expected value calculation based on shortcutting. Explicitly state that optimal post and prior solutions coincide for the deterministic setting since there is only one realized set of active vertices. Mention the a priori cost model and conclude this para by defining the obliviousness gap using a labeled equation.}

\sharat{In the next para, mention the most relevant prior work on StochTSP. Say anywhere from 3-6 papers. We are trying to tell the reader that these are well studied models. State the Shmoys Talwar upper bound of 3 and mention the lower bound of fourth root of n from CPT. You don't need to mention the polylog approximation in quasi-polytime for a priori from CPT at this point. It belongs in the related work section. Our goal is to convey that a wide separation exists between symmetric and asymmetric cases when it comes to the obliviousness gap. Now motivate the adaptive model and.}
...
\manuel{Words like obliviousness gap etc are not yet really defined but used below}
...
\begin{enumerate}
    ...
    applies to the obliviousness gap. \manuel{Perhaps it makes sense to state the previous known bound explicitly}
    ...
    $(2+\sqrt6)/4$ \manuel{$\approx 1.11$ ?} for this class.
    ...
    activations and a strictly positive asymmetric metric. \manuel{I dont know what the previous sentence is supposed to add. This is the usual model??}  Before the main
...
and \emph{asymmetric} otherwise \manuel{This definition makes every metric either symmetric or asymmetric (exactly one of the two). Usally symmetric is just considered as the special case of asymmetric}.  Unless explicitly stated otherwise, metrics
...
\begin{enumerate}
    ...
    \item \textbf{Adaptive TSP.}
        ...
        \sharat{Let's move this formalization of adaptive policies and the DP to the prelims section. They are straightforward so can be stated in prelims. In the intro define the adaptive optimum as the expected cost of a policy and point the reader to the relevant section in prelims saying that we formalize the notion of a policy in prelims.}
...
\sharat{Remove this section and make it more inline. As in, each of the gap measure should be mentioned right after it is definable. For instance, obliviousness gap can be introduced as soon as a priori model is defined. Clairvoyance and adaptivity are defined right after adaptive model is defined.}
...
\sharat{The below equation should be broken into two chunks. Once you define the a prior model you will state that post is at most prior. Then you state the adaptive model and then state the inequality that post <= adapt <= prior.}
...
\sharat{Parts of the below para should either be mentioned at the beginning of the intro or in related work.}
...
\Cref{ch:preliminaries} collects the probabilistic tools used later \sharat{and introduces some terminology for dealing with adaptive policies}.
...
\Cref{ch:asym-clair} motivates the asymmetric obstruction \sharat{of what?} through a
recursive-triangle family with ratio tending to~$4/3$, then proves the
stronger lower bound~$3/2$ \sharat{on what?} using a layered-column construction.
...
$\Omega(n^{1/3}/\log n)$ lower bound on the asymmetric adaptivity gap. \sharat{Asymmetric grid is confusing. Call it directed cylindrical grid.}
\Cref{ch:dp} presents the \sharat{presents an exponential time}  $(1{+}\varepsilon)$-approximation algorithm for a~priori TSP \sharat{for any fixed $\varepsilon > 0$}. 
\Cref{ch:conclusion} concludes with a summary \sharat{of main results in the thesis} and directions for future
research.
...
\sharat{Remove the below section heading. The content in this section should simply be a part of the chapter. It's all high level intuition. Say something like: before delving into technical details of the main theorem it is instructive to recap the ideas used for symmetric stochtsp.}

\section{From symmetric round trips to asymmetric chords}
\label{sec:asymmetric-backbone-motivation}

We begin with the master tour method of Shmoys and
Talwar~\cite{Shmoys2008}.  The factor-three formulation below is made
explicit by Blauth et al.~\cite[Section~1.1]{Blauth2024}.  First suppose that
the metric is symmetric.  Independently include every non-root vertex~$v$
in a sample~$S$ with probability~$p_v$, and call $R:=S\cup\{r\}$ the
backbone. \sharat{Would be ideal to avoid having both $R$ and $S$. Just use one of them. Preferable $S$ should already contain the root.} Let~$T_R$ be an optimal tour on~$R$.  For every~$u\in V^\circ$,
put
\[
    D_R(u):=\min_{w\in R\setminus\{u\}}d(u,w).
\]
For each unsampled vertex $u\in V^\circ\setminus S$, choose~$\mu_R(u)\in R$ such that
$d(u,\mu_R(u))=D_R(u)$. \sharat{Bad choice to use $\mu$ for nearest vertex. Use $\pi$ or $\sigma$.} Joining every such~$u$ to~$\mu_R(u)$ by two copies
of this connection \sharat{by a pair of parallel edges for every $u \notin S$ gives an Eulerian spanning graph. This Eulerian graph can be used to obtain a master tour ... } and walking through the resulting structure gives a master
tour~$M_S$.  After the actual active set is realized, inactive vertices are
shortcut.  Conditioned on~$S$, its expected cost is at most
\begin{equation}\label{eq:symmetric-master-route}
    \cost(M_S)
    \le \opt(S)
       +2\sum_{u\in V^\circ\setminus S}p_uD_R(u).
\end{equation}

Since~$S$ has the same distribution as an activation
set,
\[
    \E_S[\opt(S)]=\optpost.
\]
Then we consider the second term.
For every~$u\in V^\circ$, let~$I_u:=\1_{\{u\in S\}}$.  Notice that~$I_u$ and~$D_R(u)$
are independent.  Adding nonnegative terms gives
\begin{align*}
    \E_S\!\left[
      \sum_{u\in V^\circ\setminus S}p_uD_R(u)
    \right]
    &\le \E_S\!\left[
      \sum_{u\in V^\circ}p_uD_R(u)
    \right]\\
    &=\sum_{u\in V^\circ}p_u\E_S[D_R(u)]\\
    &=\sum_{u\in V^\circ}\E_S[I_u]\E_S[D_R(u)]\\
    &=\sum_{u\in V^\circ}\E_S[I_uD_R(u)]\\
    &=\E_S\!\left[\sum_{u\in S}D_R(u)\right].
\end{align*}
Here the equalities use linearity of expectation and the independence
of~$I_u$ and~$D_R(u)$. For a fixed nonempty sample~$S$, let~$\text{Succ}_S(u)$ be
the successor of~$u\in S$ in an optimal tour on~$R$, then we have
$D_R(u)\le d(u,\text{Succ}_S(u))$.  Summing over~$u\in S$ gives
\[
    \sum_{u\in S}D_R(u)
    \le \sum_{u\in S}d(u,\sigma_S(u))
    \le \opt(S).
\]
...
cost of every unsampled vertex leaving the backbone and returning to it is at most~$2\optpost$. \sharat{Use $\cdot$ when separating a number $2$ from $\optpost$ for clarity. Do this throughout the thesis.}
...
randomized \sharat{approximation} factor of~$7/2$.
...
variant of the grid instances introduced by Christalla, Puhlmann,
...
\begin{enumerate}
    ...
    strengthening the recursive-triangle family, whose ratio tends
    to~$4/3$; \sharat{Not the right place to mention the $4/3$ result.}
