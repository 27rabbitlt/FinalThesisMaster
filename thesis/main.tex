...
\]
\sharat{What is $m$ here? Maybe just remove the $10m+2$ vs $9m+2$ discussion. As of now $10/9$ is sufficient information.}
...
when $s$ is active.  An adaptive policy, however, must begin probing before it learns which case applies.

\sharat{The optimal tour depends on whether the semi-active vertex is active or inactive, and these two cases induce different visit orders. An adaptive policy, however, must probe $s$ before it learns which case applies.}

\sharat{At some point do mention that probing $s$ too early is also expensive if the vertex activates since we have to travel all the way to $s$ in that case.}
...
\]
\sharat{Mention a rough value for $\frac{2+\sqrt6}{4}$ so that the reader knows how it compares with $10/9$.}
...
\sharat{Think of a good replacement for the word conflict. The informal way of describing the notion is to say that the optimal visit orders are incompatible.}
...
\sharat{A better notation for the three-chain superscript is to use $\mathrm{3C}$ as opposed to $\mathrm{TC}$. The $T$ could also mean two or three or ten. I see that you have hardcoded this. Consider defining a macro that can be easily updated.}
...
\sharat{This subsection is not needed. Also reads like an LLM.}
...
\sharat{The section heading could be improved. Something like: StochTSP with a Single Semi-Active Vertex}
...
\sharat{Call this subsection as Choice of activation probability.}

\sharat{I skimmed through this chapter. The proofs are technical but the results are all plausible. They just need effort to formalize. Once someone looks at one of these figures, the result becomes apparent. They can try it for themselves. I won't be making any more comments here.}
...
\]
\sharat{Change conflict to incompatible. And be verbose while saying optimal order. Call it optimal visit order.}
...
\sharat{Why is the label from root to the blocks $w_\ell$ and not $w_k$?}
\end{remark}
...
\sharat{Move the one layered poset stage figure to the beginning of this section. Make the always active vertices as squares and the semi-active vertices as circle. For the sake of argument, suppose that we are conditioning on a uniformly random pair of semi-active vertices being active. Call them $x,y$ among $A_i, B_i, C_i, D_i$. Make a remark that in each of the 6 cases, there exists two directed paths, each containing two arcs, spanning the vertices $g^1_{i-1},g^2_{i-1},g^1_{i},g^2_{i},x,y$. Now mention that no adaptive probing strategy can infer the two directed paths correctly with probability $1-o(1)$. Walk the reader through one small case and say that the remaining cases don't work out for the same reason.}

% \sharat{This comment isn't well written so ignore it. It's alright to mention that you will be arranging these gadgets in a chain of length $L$ for some large $L$. Just make a very high level comment that you will choose the activation probabilities so that with $1-o(1)$ probability the following two conditions holds: (i)~one of the selector stages has exactly two activations; and (ii)~none of the selector stages have three or more activations. Assume this good event happens and $i^*$ is the index where exactly two activations happen. Since post opt can always choose a perfect matching with the preceding and succeeding gates at gadget $i^*$ and none of the stages see three or more activations, post opt can probe all vertices with expected cost $2+o(1)$. However, an adaptive policy has a positive probability of making a mistake. This is because the choice of the perfect matchings with the preceding and succeeding gates crucially depends on which pair of selector gates are active. Since an adaptive policy must move to a vertex that activates right after probing, the policy must now take one of two risks: (1) either probe one or two successor gates that are nearby; or (2) probe the remaining selectors to learn which one will be active. Clearly, the first option has the risk that the other active selector gate in the same stage might have need}
...
\sharat{$L$ and $p$ have not been defined so far.}
...
\sharat{Not urgent. The gates need better notation. How about calling $g_{i-1}^1$ as $X_{i-1}^+$. Same with $-$ for the other one and the $i$ gates. For the selector use $Y_i^{+,*}, Y_i^{-,*}, Y_i^{*,+}, Y_i^{*,-}$. When you do your talk it will annoy you that the gates/gadgets $g_i$ and $G_i$ sound like the underlying graph $G$. Another wild idea is to use $\overline{Y}$ for $A_i$, $Y$ for $B$, $\lambda$ for $C$, and $\overline{\lambda}$ for $D$. These are the shapes the three edges make for the selector gates where the overline indicates a mirror image.}
...
We first evaluate what \sharat{an a posteriori} a posteriori solution can achieve after all active
...
\sharat{Better chapter title is Adaptive Policies for Asymmetric \StochTSP{}. Efficient algorithm beats constructive result beats existence result. Mention this as the main theorem followed by saying that the approximation guarantee is relative to post opt, so we in fact get a bound on the clairvoyance gap as a corollary.}
...
\sharat{Add a footnote. The lower bound construction of Christalla et al. also implies the same bound on the adaptivity gap.}
...
\sharat{Above para reads like LLM. The essence is missing again. There is no need to jump straight to the asymmetric setting nor to the adaptive setting. Dedicate a para or two to the symmetric analysis of Shmoys and Talwar since we do build upon it. It's worth it to bring the reader up to speed especially as a way to compare and contrast the challenges that ones encounters in making the proof work. As you already know, we had to put in a lot of work to get the math to work out. Mention their high level idea for establishing that the obliviousness gap is at most $3$. They start with a representative sample $A$ of active vertices in the most natural way, i.e., each vertex $v$ is independently included in $A$ with probability exactly $p_v$. Now consider an optimal TSP solution on $r+A$, which we will refer to as the backbone tour. Then mention that each unsampled vertex is assigned to a backbone vertex that is closest to it. Then talk about how the master tour (a priori solution) is constructed from this. Observe that this is an existence result when it comes to factor 3. The roughly 3.5 proof can be made efficient but in a randomized way. It is best to avoid the discussion over the algorithmic implementation at this point and just focus on the constructive nature of the factor 3 gap. Then dedicate a para analyzing the factor 3 gap. Talk about how the expected distance to visit an unsampled vertex $u$ from $r+A$ can be charged to the post opt's contribution from $u$. Same for going back from $u$ to the closest backbone vertex. Use the word cumulative to highlight that the detour cost for all unsampled vertices together can be paid for in expectation using twice post opt. A nice figure will earn you bonus points.} 

\sharat{Then start a new para by saying that Shmoys Talwar strategy inspires our ideas but there are several challenges involved in making this idea work. Mention or cite the $k$ by $k$ (or $k$ by $k$ squared) grid example and mention a rough bound on post opt. Talk about how a typical backbone tour looks like and mention its cost. Now tell the reader that the shortest to-and-fro detour from an unsampled vertex to a backbone vertex is always roughly the width of the grid. And that this loss in approximation is also true if we made our probing decisions adaptively with the restriction that detours always must return to the backbone vertex where they started from.}
...
\sharat{Each idea in this chapter is significant enough to be a section by itself. Dedicate a section to the circulation lower bound. Make sure to convey the intuition of fractionally assigning detours for unsampled vertices to directed chords of backbone tour. It is very important to convey the message that we are giving a new approximate lower bound on post opt that can be leveraged to design our adaptive policy. Start the next section entitled Assigning Detours to Backbone Chords. Now talk about the rounding idea with a full proof.}
...
outside a sampled backbone to a tour on that backbone. 
\sharat{Attaching is ambiguous. Say that probing a non-backbone vertex might result in an activation, which warrants a detour from the backbone.}
We then round the
...
\sharat{We could mention our example with two permanent and n temp vertices. distance between temp vertices is twice root n. distance from left perm to any temp vertex is 1 and the same from temp vertex to the right perm vertex. distance from right perm to left perm is n. post opt is roughly n. if we greedily choose detours to start from the closest preceding backbone vertex and end at the closest succeeding backbone vertex then the detours cost 2. while these detours look cheap (as the cumulative detour in expectation is root n, while post opt is n) they induce a huge mismatch in the in vs. out degrees of the left perm and right perm vertices. To circumvent this, we enforce circulation constraints. This is a reasonable thing to do because post solution is a circulation on the active vertices. You could mention the example as a remark after stating the circulation LP or the circulation lower bound theorem. Saying that the circulation constraints are very important. Removing them leads to an arbitrary loss in approximating post opt. Note that the 1 could be replaced by a tiny epsilon so the greedy detour cost per vertex would be twice epsilon.}
...
\]
for a universal constant~$C_{\mathrm{back}}$. \sharat{I am not in favor of this universal constant business. Simply write $O(\log n)$ in the RHS. Same for the main theorem. Big O notation implicitly encodes that there is a universal constant.}
\end{lemma}
...
\sharat{As mentioned earlier. This is a section by itself. You do not have to necessarily talk about adaptive policies at this point. The story you are telling is that there is a certain augmented chord-backbone structure that encodes post opt up to polylog factors. As long as the operations only lose polylog relative to post opt, you don't need to allude to adaptive optimum.}
...
\sharat{Better section heading is Adaptive Probing of Rare Vertices. Since you are explicitly focusing on them.}
...
\sharat{Not a fan of working with explicit constants for backbone and rounding losses.}
\end{proof}

\section{Threshold optimization and the clairvoyance-gap bound}
\label{sec:asymmetric-threshold-optimization}

\sharat{The section heading makes the choice of $\tau$ and the clairvoyance bound implication appear more important than they are. The choice of $\tau$ should be apparent to anyone who stares at the competing terms going into the analysis. Oftentimes people title such a section as say An $O(n^{1/3})$-Approximate Adaptive Policy since this is where the full description of the policy for a general instance is given.}
...
\sharat{This needs to be replaced with the official declaration document.}
