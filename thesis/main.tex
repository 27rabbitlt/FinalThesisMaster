...
\sharat{visited not served} served at minimum expected travel cost.

\sharat{Only the a posteriori and a priori solutions can be called benchmarks as they are existing models that have been studied a lot. Our model of adaptivity is new. Make this para as a discussion between the two extremes. Do mention that for symmetric metrics the gap between these two benchmarks is a constant, whereas it is polynomial for asymmetric case. Then end the para by saying that the thesis introduces a new model of adaptive TSP with the goal of understanding whether adaptivity can help with obtaining better stochastic tours.}
...
\sharat{Since your thesis is quite long, it makes sense to group the results into two paras. First you can talk about the gap results that are based on various constructions. It's your call whether you want to state every single result explicitly or just that you obtain new/improved lower bounds for every single setting: obliviousness gap for both, adaptivity for both, clairvoyance gap for both and single stochastic vertex case. You should only state the best possible results for each. The second para can be dedicated for algorithmic results. Talk about the $\tilde{O}(n^{1/3})$-approximate adaptive policy first. Then mention the a priori DP.}
...
\begin{enumerate}
    ...
    The algorithm discards pairs for which the probability that all \sharat{rephrase. intervening doesn't have the correct ring.} intervening clients are inactive is tiny, losing only a negligible amount, and then uses a dynamic program whose state stores just the relevant suffix of the partial order.
\end{enumerate}
...
\sharat{Directed metric is wrong/non-standard usage. Just say metric. Rephrase: A metric $d$ is said to be \emph{symmetric} if blah happens. Replace the otherwise part by saying something like: unless specified, metrics are assumed to be asymmetric.}
...
\begin{enumerate}
    \item \textbf{A posteriori TSP.}
        \sharat{It's best to avoid talking about a post solutions as an algorithmic question because the problem in itself is NP hard. Maybe emphasize that $T_A$ has cost $\opt(A)$. The reason for this is quite simple: an entity with clairvoyance can be assumed to also have unbounded computational power.}
        ...
        \]
        Since each $T_A$ can be chosen independently \sharat{don't use the word independently outside of stochastic context. find a synonym that doesn't add to the confusion. Ideally I would just define $T_A$ to be an optimal tour on $A$ and be done with it.}, the optimum is
        \[
    ...
    \item \textbf{A priori (nonadaptive) TSP.}
        ...
        $V^\circ=\{v_1,\ldots,v_n\}$. \sharat{Calling an a priori tour as a tour is confusing. Maybe say something like: An a priori tour is a cyclic permutation of all vertices that starts and ends at $r$.}
...
\sharat{Maybe replacing intervening with intermediate or inner or blah that comes in between so and so is better. Do this throughout the thesis.}
...
endpoints are active and every \sharat{intermediate} intervening vertex is inactive.  This exact
decomposition identifies the low-probability terms that can later be
discarded.

\sharat{It's not a good practice to write simple things as equations on a new line. $T=(v_0 = r,v_1,\dots,v_n,v_{n+1} = r)$ can be written inline.}
Let
\[
    T=(r,v_1,\dots,v_n,r)
\]
be an a~priori tour, where $V^\circ=\{v_1,\dots,v_n\}$. \sharat{For any $0 \leq i < j \leq n+1$, we use the shorthands $p_i$ and $d_{ij}$ to denote $p_{v_{i}}$ and $d(v_i,v_j)$, respectively, and we define $Q(i,j):=\prod_{i<k<j}(1-p_k)$, with the convention that an empty product equals $1$.} Put
...
For \sharat{a threshold parameter $\eta \in (0,1)$, blah} $0<\eta<1$, define the truncated cost \sharat{of an a~priori tour $T$ to be the RHS of \eqref{eq:apriori-pairwise} without the pairs $(i,j)$ for which $Q_{i,j} \leq \eta$. That is:} by retaining only pairs for which
$Q(i,j)>\eta$:
...
\sharat{The following lemma shows that the choice of the threshold parameter can be used to control the approximation loss between the (full) cost and the truncated cost. The stuff that you do for the final proof should be done here. If you want a $(1+\epsilon)$ loss between truncated and full cost, then you should say that taking $\eta = \Omega(\epsilon)^{O(1/\epsilon)}$ does the job.}

\sharat{Use "for every tour" when you state the lemma in a ratio form and the RHS is a uniform upper bound on all a priori tours. Use "for any tour" when both the LHS and RHS have a dependency on the tour.}

\sharat{$\kappa$ is unnecessary in the lemma statement. It's OK for an expression to look complicated, but an equation should never give the impression that there are more degrees of freedom than intended. \eqref{eq:long-gap-truncation} makes it look like there are 3 parameters.}

\sharat{I might have a way to ensure that $\eta$ only depends polynomially on $\epsilon$. ChatGPT wrote a proof that I haven't verified yet. See pairwisetruncationetaepsilonsquared.pdf in the thesis folder.}

\begin{lemma}[Pairwise truncation bound]
\label{lem:long-gap-truncation}
\sharat{Rephrase: Let $\epsilon \in (0,1)$ be an error parameter and let $\eta \in [0,\epsilon^{1/\epsilon}]$. 
For any a~priori tour~$T$, we have: \[ \cost(T)
    \le (1+\epsilon) \cost^{\loc}_{\eta}(T).\]
}
...
\sharat{Rephrase: The coupling argument is straightforward because you described a strategy for always ensuring that $\widetilde A \supset A$. That's what coupling is. You don't need to give the exact mapping from probability measure of $p$ to that of $\widetilde p$. Also, put the entire coupling argument in a single para. Any notion that is small enough to be explained using at most 5-6 sentences should be in a single para.}
...
\[
    ...
    \le(1+\alpha)p_v.
\]
Moreover, \sharat{$\widetilde Q$ is a definition. Use $\coloneqq$.}
\[
    \widetilde Q(i,j)
...
inactive products of the complete \sharat{intermediate} intervening blocks but omit the two
...
$x<_\tau y$, the inactivity factor contributed by the vertices of~$B$ \sharat{strictly}
...
\sharat{Label these endpoints as say $x$ and $y$ for clarity.}
...
\sharat{the vertices in $B_i$ that come after $x$} the vertices after the first endpoint in~$B_i$, all complete blocks strictly
between $B_i$ and~$B_j$, and \sharat{the vertices in $B_j$ that come before $y$} the vertices before the second endpoint
...
product over complete \sharat{intermediate} intervening blocks.  To aggregate the remaining base
...
of the complete intervening blocks. \sharat{Inactivity probability doesn't sound good. Use intermediate, middle or inner instead of intervening.}
...
\sharat{$\alpha$ and $\kappa$ should not appear in these calculations. You simply pick an error parameter $\epsilon$ and then $\eta = \epsilon^2$ should get the job done.}
...
\]
\sharat{The first inequality holds by the definition of $I(B)$ (see \Cref{eq:internal-block-cost}).}
...
most~$1$ \sharat{from a heavy vertex I am assuming?}.  Thus the total activation mass of a relevant suffix is less than
\[
    M_\eta:=\log(1/\eta)+1.
\]
...
\sharat{This proof needs clean up once the truncation loss lemma becomes more direct.}
\end{proof}
