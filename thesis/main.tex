...
\sharat{A general comment for the abstract is to avoid using notation or terminology that has not been defined. I will give you suggestions below.}
...
\sharat{We study the stochastic traveling salesperson problem on a finite set of vertices equipped with distances satisfying the triangle inequality, where
there is a designated depot and every other vertex is independently active
with a given probability. The salesperson's goal is to visit all active vertices, starting and ending at the depot, while minimizing the expected total distance traveled.}
...
\sharat{An a~posteriori solution chooses a tour after knowing the set of active vertices, whereas an a~priori solution fixes an ordering of all the vertices in advance and visits the active vertices according to this ordering. The ratio between the optimal expected costs in these two models is bounded by a constant for symmetric metrics (Shmoys and Talwar, 2008), but can grow polynomially with the number of vertices for asymmetric metrics (Christalla, Puhlmann, and Traub, 2026). This thesis introduces a novel adaptive model between these extremes, in which vertices are probed sequentially to reveal whether they are active, with the requirement that the salesperson must visit a vertex if probing reveals that it is active. The choice of the next vertex to probe may depend on the outcomes of previous probes.}
...
approaching~$10/9$; for instances with one semi-active vertex, we also prove the uniform upper bound $(2+\sqrt6)/4$ on the clairvoyance gap.  For asymmetric metrics, we give a layered-poset construction showing that the clairvoyance gap is at least~$3/2$.

\sharat{For symmetric metrics, we construct instances in which the ratio between the optimal a~priori and adaptive costs, which we call the \emph{adaptivity gap}, approaches 2, and instances in which the ratio between the optimal adaptive and a~posteriori costs, which we call the \emph{clairvoyance gap}, approaches 10/9. For instances in which only one vertex has activation probability strictly between 0 and 1, we prove that the clairvoyance gap is at most blah. For asymmetric metrics, we improve upon Christalla et al.'s lower bound of blah on the adaptivity gap to blah, and construct instances in which the clairvoyance gap approaches 3/2.}
...
\sharat{On the algorithmic side, for asymmetric metrics we construct an adaptive policy whose expected cost is within an $O(blah)$ factor of the optimal a~posteriori cost, where $n$ is the number of vertices. I think you forgot to include polylog terms. Use $\widetilde O$ notation if need be. Consequently, the clairvoyance gap is at most $O(n^{1/3})$. For every fixed $\varepsilon>0$, we also give a deterministic algorithm
that computes a $(1+\varepsilon)$-approximate a~priori solution in
time $c_\varepsilon^n\operatorname{poly}(n)$, where $c_\varepsilon$ depends only on $\varepsilon$.}
...
\sharat{Maybe include a footnote that in mathematics literature asymmetric metrics are called quasi-metrics.}
...
$X:=\sum_iX_i$, and write $\mu:=\E[X]$.  We use the additive
Chernoff--Bernstein bounds
\begin{align}
    \P(X\ge\mu+s)
...
\begin{equation}\label{eq:chernoff-useful}
    ...
    \P(X<\mu/2)\le\e^{-\mu/8}.
\end{equation}
The first consequence follows from \cref{eq:chernoff-upper} with
$s=\mu+t$, using
\[
...
\sharat{Should be mentioned as a theorem or a lemma.}

We also use the following weighted form of Hoeffding's inequality.  Let
...
\sharat{Looks technical. Should be a named theorem.}

Let $Z_1,\ldots,Z_m$ be independent centered random variables with finite
second moments, and put $S_k:=\sum_{i=1}^kZ_i$.  The $L^2$ maximal inequality
for the martingale $(S_k)_{k=0}^m$ gives
...
chapter we assume that $n\ge2$.

This improves the $O(\sqrt n)$ bound implied by the asymmetric
...
This section develops all static ingredients needed by the adaptive policy.
...
Call an unsampled vertex an \emph{outside vertex}.  We require every outside
vertex~$v$ to have fractional in-degree and out-degree~$p_v$, and forbid flow
between two outside vertices.

Independently include each vertex $v\in V^\circ$ in a random set~$S$ with
...
For a nonnegative vector~$z$ on ordered pairs, use the notation
\[
...
\]
The circulation does not allow flow between outside vertices,
forcing that it visits an outside vertex  from the backbone and immediately returns to the
backbone.  Thus the circulation is a fractional analogue of assigning
a detour to each outside vertex.

We next bound the circulation cost on average over the sampled backbone.
...
independent of~$S$ and using the same probabilities
$(p_v)_{v\in V^\circ}$, and put $C:=\widetilde A\cup S$.  For every possible
set~$C$, fix one optimal rooted tour~$T_C$, with tie-breaking depending only
on~$C$.  Cutting this tour at the vertices
...
We bound the expected cost of this construction through the longest segment
...
\end{align*}
where $c_{\mathrm{back}}$ is universal.  The penultimate inequality follows by
...
learns which vertices are active.  We obtain both objects in one randomized
rounding procedure, independent of the activations.  It preserves their
combined cost in expectation and leaves only $O(\log^2 n)$ imbalance on every
...
\end{equation}
where $C_{\mathrm{round}}$ is universal.  Moreover, if~$\rho$ denotes the
internal randomness of the procedure, then
\begin{equation}
    \E_\rho\!\left[
      \sum_{i\in U}p_i
...
\]
These bounds are fixed throughout the procedure.
We preserve every outside-vertex equation
\begin{equation}\label{eq:joint-rounding-vertex-equation}
    \sum_{a,b\in R}\xi_{i,ab}=p_i.
\end{equation}
We also preserve selected interval equations
$\Delta_{\xi,\eta}(D)=0$.

Pad the linear order $s_0,\ldots,s_{m-1}$ with empty positions to the next
power of two, and let~$\mathfrak D$ be the dyadic interval family given by
the resulting binary tree.  We identify each tree node with its intersection
with the actual backbone positions, ignoring empty padded leaves.  Put
\[
    h:=\lceil\log_2m\rceil,
    \qquad
    L:=8(h+1).
\]
Call a variable \emph{live} when it lies strictly between its two bounds.
An interval equation is retained while at least~$L$ live variables have
nonzero coefficient in it, and is discarded permanently once fewer than
$L$ live variables touch it.  Initially all interval equations hold by
\cref{eq:adaptive-fractional-interval-balance}.

Let $F_x$ and~$F_y$ be the sets of live $\xi$- and $\eta$-variables, and put
$F:=F_x\cup F_y$.  If $F=\emptyset$, terminate.  Otherwise, let~$c_1$ be the
number of outside-vertex equations containing a live variable.  Such an equation
cannot contain exactly one live variable:
...
\]
Let~$c_2$ be the number of retained interval equations.  Each backbone
position belongs to at most~$h+1$ dyadic intervals, so every chord variable
has nonzero coefficient in at most~$2(h+1)$ such equations.  Counting
incidences gives
\[
    c_2L\le2(h+1)|F|,
...
equalities has a nonzero direction supported on~$F$.

Move maximally in both signs of this direction until a live variable first
...
\[
    ...
    q^-:=q-\beta d,
\]
where~$q$ is the current vector of live variables and~$d$ is the chosen
...
equalities and make at least one additional variable nonlive, while
\[
...
Consider a dyadic equation when it is discarded.  Its imbalance is still
zero, and fewer than~$L$ live variables touch it.  Only those variables can
subsequently change.  A $\xi$-variable changes by at most~$p_i\le1$, and an
$\eta$-variable changes by at most one because its bounds are consecutive
integers.  Therefore its final imbalance has absolute value at most~$L$.
Every linear interval has a canonical decomposition into $O(h+1)$ disjoint
dyadic intervals.  A wrapped cyclic interval is the complement of a linear
interval, and the imbalance of~$R$ is identically zero.  Additivity therefore
gives final discrepancy $O((h+1)L)=O(\log^2 n)$ for every cyclic interval,
which proves~\cref{eq:joint-rounding-interval}.
\end{proof}

The joint rounding thus leaves only $O(\log^2 n)$ deterministic imbalance
...
rounded objects into a causal adaptive policy.
...
For every realization~$A$, put $F:=F(A)$ and define its augmented maximum
...
\]
The policy described above uses every chord in~$F$ exactly once and every
...
Let $c_e$ be the number of traversals of backbone edge~$e$.  Flow balance of
the completed closed walk over any cyclic interval implies that the
difference of the traversal counts of its two boundary backbone edges is
...
has multiplicity one.  Every other backbone edge therefore has multiplicity
at most~$\Lambda(F)$.
\end{proof}
...
\end{align*}
The absolute value of the first line is at most
...
\[
    ...
    \le\mu.
\]
For a backbone prefix $J_k:=\{s_0,\ldots,s_k\}$, write
\[
    Z_k:=\sum_{i\in U}X_i\chi_{J_k}(a_i,b_i)
    =\sum_{i\in U}X_i\1_{\{a_i\in J_k\}}
     -\sum_{i\in U}X_i\1_{\{b_i\in J_k\}}.
\]
Order the outside vertices once by the positions of their chord tails~$a_i$,
breaking ties in an arbitrary fixed way.  The first process on the right,
evaluated at prefix boundaries, is a subsequence of the partial sums in this
order.  Hence
the $L^2$ maximal inequality
\cref{eq:l2-maximal} gives
\[
    \E_A\max_k
      \left|\sum_{i\in U}X_i\1_{\{a_i\in J_k\}}\right|
    \le2\sqrt\mu.
\]
Ordering the outside vertices by their chord heads~$b_i$, again with fixed
tie-breaking, gives the same bound for the second process, and therefore
\[
    \E_A\max_k|Z_k|\le4\sqrt\mu.
\]
Every linear interval is the difference of two prefixes, while a wrapped
cyclic interval is the complement of a linear interval and
$\chi_R(a,b)=0$.  Consequently,
\[
    \E_A\max_I
      \left|\sum_{i\in U}X_i\chi_I(a_i,b_i)\right|
    \le8\sqrt\mu.
\]
Combining this estimate with the pathwise discrepancy bound for the first
line yields, for every~$(S,\rho)$,
...
Put
\[
    L:=1+C_{\mathrm{round}}\log^2 n+8\sqrt\mu.
\]
For fixed preprocessing~$(S,\rho)$, consider first the history-dependent
virtual strategy described above.  By \cref{lem:active-first-stack-walk},
each chord copy is used once and every backbone edge is used at most
$\Lambda(F(A))$ times.  Its actual route is a shortcut of the expanded
virtual walk, so its cost for realization~$A$ is at most
\begin{align*}
...
\end{align*}
By the Markovization observation in \cref{sec:stoch-tsp-benchmarks}, there is
a formal adaptive policy~$\pi_{S,\rho}$ with no larger expected cost.  Hence
\begin{align*}
    \cost(\pi_{S,\rho})
    &\le
      \sum_{a,b\in R}N_{ab}d(a,b)
...
\cref{eq:joint-rounding-cost,eq:adaptive-active-imbalance} yields, for each
fixed~$S$,
\[
    \E_\rho[\cost(\pi_{S,\rho})]
    \le \cost(z^S)+L\opt(S).
\]
Put $B:=\optpost(W_\tau)$.  Averaging over~$S$ and using
\cref{lem:average-backbone-cost} now gives
\begin{align*}
    \E_{S,\rho}[\cost(\pi_{S,\rho})]
    &\le \E_S[\cost(z^S)]+L\E_S[\opt(S)]\\
    &\le \bigl(C_{\mathrm{back}}\log n+L\bigr)B\\
    &\le C_3\bigl(\log^2 n+\sqrt\mu\bigr)B
\end{align*}
...
This conclusion also covers $B=0$, since then the nonnegative average above
is zero.
\end{proof}
