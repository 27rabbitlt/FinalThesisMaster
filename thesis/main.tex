...
approaching~$10/9$; for instances with one stochastic vertex, we also prove the
...
$O\bigl(n^{1/4}\sqrt{\log(n+1)}\bigr)\optpost$.  For every fixed
...
\begin{enumerate}
    ...
    We first define an asymmetric recursive family whose ratio
    ...
    activations and a strictly positive asymmetric metric.  We also give an
...
least~$10/9$.  We construct a family of instances with one stochastic
...
bound to~$3/2$.  It embeds a layered poset into an asymmetric metric: a
clairvoyant tour needs essentially two increasing chains, while every causal
policy needs at least three increasing runs with high probability. \sharat{It's my first time hearing causal policies. They haven't been defined. Sounds like LLM talk. Try to write this para in your own words. Only allow LLMs to rephrase the grammar and not the technical part.} Both
proofs allow a policy to interleave probes across different recursive blocks or
poset layers.

\sharat{There should only be two sections in this chapter.}
...
$(\mathcal{I}_t^{\mathrm{LP}})_{t\ge2}$ on strictly positive asymmetric metrics
such that
\[
    \liminf_{t\to\infty}
    ...
    \ge\frac32.
\]
\sharat{independently activated instances is unnecessary. That is all we deal with in this thesis. strictly positive asymmetric metrics is also unnecessary. it's not adding any new information. you can take any instance of StochTSP and perturbe it slightly }
\end{theorem}
...
\subsection{Embedding the poset into an asymmetric metric}
...
an asymmetric metric, attaching a vertex to its nearest backbone vertex is not
