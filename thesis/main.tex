...
\begin{enumerate}
    ...
    \item \textbf{Clairvoyance gap for symmetric metrics} (Chapter~\ref{ch:sym-clair}).
    ...
    that setting is~$10/9$ (Theorem~\ref{thm:sym-clair}).
...
\begin{enumerate}
    ...
    \item \textbf{Adaptive TSP.}
        ...
        During execution, the salesperson \emph{probes} clients one by one.
        A probe is performed remotely and reveals whether the probed client is
        active before the salesperson moves.  If the probed client~$v$ lies
        in~$A$, the salesperson \emph{must} travel to~$v$ immediately;
        otherwise the salesperson remains in place.  Informally, a probe can
        be viewed as remotely calling the client.  The order of probes may
        depend on the outcomes observed so far.

        Formally, an adaptive policy is a function $\pi: (\{r\} \cup V) \times 2^V \rightarrow \{r\} \cup V \cup \{\bot\}$, where $\pi(u,S)$ denotes the next vertex to probe when the salesperson is at~$u$ and the set of unprobed vertices is~$S$.
        We require: (a)~$\pi(u,S) := \bot$ whenever $u \in S$ (invalid input); (b)~$\pi(u,S) = r$ iff $S = \emptyset$ (return to depot only when done); (c)~$\pi(u,S) \in S$ iff $S \neq \emptyset$ (must probe an unprobed vertex).
...
Chapter~\ref{ch:sym-clair} proves a symmetric clairvoyance-gap lower bound
of~$10/9$ and determines the exact gap for one stochastic client of
...
% ============================================================
\chapter{Adaptivity Gap for Symmertic StochTSP}\label{ch:sym-adapt}
...
\sharat{I recommend introducing an acronym of StochTSP for stochastic tsp. Use a macro for this. It's your call whether to introduce SymStochTSP and AsymStochTSP. I would avoid introducing long shorthands and just refer to them as symmetric/asymmetric StochTSP instances.}

\sharat{Rephrasing of the intro para for this chapter. The main result in this chapter is that the adaptivity gap for symmetric StochTSP is at least $2$. This chapter is divided into two sections. In \Cref{sec:blah}, we establish a weaker lower bound of $5/4$ on the adaptivity gap via an explicit construction of StochTSP instances arising from cycle-subdivided cliques. The stronger lower bound of $2$ is established in \Cref{sec:blah} using the probabilistic method on $(1,2)$-TSP instances induced by a random graph $G_{n,p}$ in the Erdős–Rényi model.}

\sharat{Some advice on the writing process. Imagine the process of describing high-level ideas as a breadth first search procedure. You can apply this meta rule at all levels: chapters, sections, subsections, proofs, etc. When you are at the level of this chapter, all that matters is the two results of $5/4$ and $2$ and not that a certain ratio is tending to these numbers. You don't want to just state these numbers but also give the reader some context on why we chose to have two sections instead of just one for the stronger result. There's a qualitative difference between the two results. The $5/4$ bound is an explicit construction whereas the $2$ bound is probabilistic. It's good to give a teaser to the reader without bombarding them with technical details. The numbers $5/4$ and $2$ and the terms explicit vs probabilistic are still not descriptive enough for what we are going to accomplish. So it makes sense to say more about these constructions without being too precise. Cycle-subdivided clique is fairly descriptive in the sense that a reader can visualize the subdivision operation on a cycle in a clique. Same for the $G_{n,p}$ model. It tells the reader that you are going to work with Erdos Renyi model where edges are independently present in the graph. Since we also say $(1,2)$-TSP, we are alluding to the reader that we are going to put a label of $1$ or $2$ depending on whether an edge appears or not. The advantage with this style of writing is to get the reader hooked. Your goal is to give them just enough hints so that they can convince themselves why the result goes through. If the reader can prove the result by themselves, then they will appreciate your work more and forever remember your thesis.}

\section{The cycle-subdivided-clique construction}
\label{sec:cycle-subdivided-construction}

%Sub(K_n;C_n)
%no need to state sym again
% there should be a sectence between theo and corollary. should not list theorems in this way.
\begin{theorem}\label{thm:cycle-subdivision}
The adaptivity gap for symmetric stochastic TSP is at least $\frac54$.
\end{theorem}
%move the corollory to 2 lower bound.
...
$K_n$, with indices taken modulo~$n$.  We call
$C_n=(b_1,\dots,b_n,b_1)$ the \emph{base cycle} and its vertices the
\emph{base vertices}.  For every $i\in[n]$, subdivide the edge
$b_ib_{i+1}$ of the base cycle by introducing a \emph{subdivision
vertex}~$s_i$. Denote the resulting unweighted graph by~$G_n$.
\end{definition}
% we just use vertex set and depot is a special vertex. we also call depot vertex to be root vertex (r). in technical part, use root and vertex; in intro use depot and client
The \emph{cycle-subdivided-clique instance} $\mathcal I_n$ has depot $r:=b_1$ and client set
\[
V_n:=\{b_2,\dots,b_n\}\cup\{s_1,\dots,s_n\}.
\]
The underlying distance function is the shortest-path metric induced by~$G_n$. Every
base client $b_2,\dots,b_n$ is active with probability~$1$, while every
subdivision vertex~$s_i$ is active independently with probability~$1/2$.

\begin{figure}[htbp]
\centering
\begin{tikzpicture}[
    scale=1.05,
    base/.style={rectangle,draw=black,fill=white,minimum size=8mm,inner sep=0pt,font=\small},
...
\foreach \i in {1,...,6} {
  \node[base] (b\i) at (c\i) {$b_\i$};
  \node[stochastic] (s\i) at (m\i) {$s_\i$};
}
\node[above=2mm of b1,font=\small] {depot $r$};
\end{tikzpicture}
\caption{The cycle-subdivided clique $G_6$. Base vertices are active with
...
% convert it to subsection of 2.1
\section{The a priori optimum}
\label{sec:cycle-prior}
% short paragraph: in this section we show lemma 2.4,to do this we consider an arbitrary a priori solution, which can be represented as a permutation
...
two base vertices.  Summing these local bounds gives a global lower
...
\end{equation}
For a subdivision vertex and a base vertex,
...
\end{claim}
% we need to say somewhere that we treat vertex index in modulo, n+1 is just 1
% subdivision vertices or subdivided vertices
% or maybe semi-active vertices
\begin{proof}
...
A two-vertex realization can have cost~$4$ only if one active subdivision vertex is
incident with~$u$, the other is incident with~$v$, and the corresponding
base-cycle edges are adjacent.  For fixed nonconsecutive $u$ and $v$, there is
...
subdivision vertex is the unique subdivision vertex of the edge $uv$ and the other comes
from one of the two base-cycle edges adjacent to $uv$.  Thus among the three
...
\]
By~\eqref{eq:cycle-base-distances}--%
...
\]
Since a base vertex is incident with exactly two subdivision vertices,
\[
    \sum_{t=1}^k2^{-t}\alpha_t
    \le\frac34,
    \qquad
...
corresponding subdivision vertices arise from adjacent edges of the base
...
\]
on $\{b_1\}\cup V_n$, where $x_1,\dots,x_{2n-1}$ is a permutation of
$\{b_2,\dots,b_n,s_1,\dots,s_n\}$.  Deleting all subdivision vertices
from~$T$ leaves a cyclic ordering
\[
    (b_1=u_1,u_2,\dots,u_n,b_1)
\]
of the base vertices.  For each $j\in[n]$, let
\[
    W_j=(w_{j,1},\dots,w_{j,k_j})
\]
be the ordered list, possibly empty, of subdivision vertices appearing
between $u_j$ and $u_{j+1}$, where $u_{n+1}:=b_1$.  Every subdivision
vertex belongs to exactly one such segment, so
\[
    \sum_{j=1}^n k_j=n.
\]
Because all base vertices are active with probability~$1$, the realized
...
between base vertices, each of length~$1$.  Hence
\[
...
\[
    ...
    \qquad(2\le t\le m),
\]
each contributing $11/4$, and $m+1$ direct edges between base vertices,
...
\]
viewed as an ordered path.
%alternating sweep policy
\medskip
\noindent\textbf{Policy.}
\begin{enumerate}
    \item Probe $s_1$, then $s_n$, and then $b_2$.  The last probe leaves the
    salesperson at~$b_2$.  Set the sweep direction along~$P$ from $b_2$
    toward~$b_n$.

    \item While an unprobed client remains on~$P$, maintain the following
    invariant: the unprobed clients form a contiguous subpath, and the
    salesperson is at a probed base vertex adjacent to one end of it.
    Probe the subdivision vertex at that end.
    \begin{enumerate}[label=(\alph*)]
        \item If it is active, travel to it.  If a base vertex remains
        unprobed in the sweep direction, probe that vertex and continue in
        the same direction.
        \item If it is inactive, remain in place.  If an unprobed client
        remains, probe the base vertex at the opposite end of the remaining
        subpath and reverse the sweep direction.
    \end{enumerate}

    \item After every client has been probed, return to the depot~$r$.
\end{enumerate}

\medskip
\noindent\textbf{Cost analysis.}
The first three probes cost at most
\[
...
\]
Each of the $X$ active subdivision vertices on~$P$ causes at most two
unit-length moves: one to the subdivision vertex and one to the next base
vertex.  An inactive subdivision vertex can cause one switch to the base
vertex at the opposite end of the remaining subpath.  Every such switch
costs~$1$, except possibly the last, which may cost~$2$ when its endpoints
...
\sharat{The below three paras read like an LLM wrote it. Try to give it your own voice because the current text is not illuminating. It's OK if the flow isn't as smooth but words like pattern and continuation seems out of place.}

\sharat{You don't have to necessarily force a connection with the previous section. Try to give intuition through $G_{n,p}$. Mention that the probability of activation is small. Mention that the graph is symmetric in its structure so we only need to show that the probability that the a priori tour $1,\ldots,n$ has low cost is much smaller than $1/n!$. Say something about why the adaptive policy has a decent chance of paying a cost of $1$ to visit the next activated vertex. This story will take some time to clean up so give it another attempt.}
...
policy, by contrast, can use probe outcomes to decide which cheap
...
independent activation randomness.  An adaptive policy can instead probe all
cheap neighbors of its current location before probing a client at distance~$2$.
...
All logarithms in this section are natural \sharat{all logs are to the base $\mathrm{e}$}.  Fix a sufficiently large
...
otherwise. 
\sharat{It's a good idea to mention that this random metric is an instance of the $(1,2)$-TSP problem. Every realization being a symmetric metric is trivial. You shouldn't have to establish it.}
We call a pair of distance~$1$ a \emph{cheap edge}.
...
\sharat{I don't understand why $n/\log^5 n \to \infty$. It's not relevant at this point to take $n \to \infty$.}
...
\]
and a deterministic realization~$d$ of the metric. \sharat{Rewrite this section. Written in the style of an LLM. You have to recall definitions of a priori cost by pointing the relevant definition or equation.} Write
\[
...
\begin{proof}
\sharat{You only need to argue that the a priori tour $r,1,\ldots,n,r$ has low cost with extremely tiny probability.}
...
\sharat{The above section does not read well. The intuition for the proof is missing. Some quantities are introduced without motivating why they are relevant. Mention Hoeffding inequality in the prelims chapter and invoke it with the correct parameters. Also, whenever you invoke a lemma, you have to mention why the hypothesis holds. Make a remark early on that the cost of the tour is a sum of independent bounded random variables with mean so and so.}

\subsection{A cheap adaptive policy}

\sharat{Notation is quite poor. You need to recall how $d$ is obtained. Since it's a random variable, you will need to introduce more precise notation.}
For each deterministic metric realization~$d$, define a policy~$\pi_d$ as
follows. 
\sharat{Best to avoid using client terminology.}
Suppose the salesperson is at~$u$ and the set of unprobed clients
is~$S$.  If $S=\varnothing$, return to~$r$.  If $u$ has a cheap neighbor
in~$S$, probe the \sharat{vertex with the smallest label. mention early on that the vertices are labeled $1,\ldots,n$.} least-labeled such client; otherwise probe the least-labeled
client in~$S$.  Thus all unprobed cheap neighbors of the current location
are probed before any client at distance~$2$.

\sharat{Confusing text.}
...
\sharat{Maybe name the adaptive policy as \emph{nearest unprobed neighbor}. I am intentionally avoiding mentioning lazy in the name because the actual implementation of the policy can also be done non lazily.}
...
arrives at an active client~$u$, let $S$ be the set of unprobed clients and
suppose that exactly $s$ clients in~$S$ are active.

\sharat{This part is a bit confusing as the metric was claimed to have been realized. You need to rethink how you present this section. since we are establishing an upper bound, it suffices to prove that some adaptive policy has low cost in expectation. Describe the adaptive policy precisely. Right after the description prove a lemma which captures the invariants of the adaptive policy. For example, if the adaptive tour has just arrived at an active vertex $u$ and the set of unprobed vertices is $S$, then none of the edges in $G[u+S]$ have realized their lengths. You could say that the distribution of edge lengths is identical to $G_{1+|S|,p}$ in your cost model. To do this you need to setup some clean notation early on about the distribution of edge lengths for a $k$-vertex graph.}
...
At the initial state $u=r$, the edges from the depot to all unprobed clients
...
\sharat{The above calculation is focusing on the wrong details. The probability that $K$ is nonzero is essentially $1$. Optimizing irrelevant stuff at this level while sacrificing readability is a signature move unique to LLMs.}
...
probed clients are deterministic.  Therefore the active and inactive
...
the entire active expanded walk, so all probes of its deterministic clients must
occur in its unique component interval.  Moreover, the crossing by which the
walk enters that interval must occur before the probe of $s$.  Indeed, if the
entry crossing occurred only as part of the post-probe movement to the active
vertex $s$, none of the clients of the preceding chain could have been probed
in that interval before $s$; serving them later would require re-entering the
chain and would create additional crossings.

Consequently, before the probe of $s$, the common expanded prefix has already
used either
\[
  a_mc_0
  \qquad\text{or}\qquad
...
set of deterministic clients.  If $D=\varnothing$, then probing $s$ at the
...
the policy is in an abstract exterior state.  It must probe every client of~$C$
exactly once.  Probes returning an inactive outcome before the first movement
leave the policy in the exterior state.  When the first active client~$v$ is probed, the policy
...
apply with metric~$d_C$.  Once all clients have been probed, the policy chooses
...
it into this walk.  Passing through a client in an expanded walk neither probes
...
$M$ is probed.
\begin{itemize}
    \item If $M$ is probed at the initial depot, an active execution needs at
...
\end{itemize}
There is no case in which $M$ is probed after a final return to~$D$, because
the policy may return only after all clients have been probed.  Consequently,
...
client~$v$ is probed, concatenate the pending exit walk, the prescribed
...
predecessor gate in stage~$i$, probing the active client~$Y$ jumps past an unserved
...
argument preceding Lemma~\ref{lem:four-star-local}.  The first premature probe
...
\[
    ...
    \le1-h,
\]
where $h\le q$.  Further premature probes start from potential~$1$ and have
...
to zero.  Hence
\eqref{eq:four-star-bellman} and the preceding premature-probe estimate make
...
and leaves only $O(\log(n+1))$ \sharat{The difference between $\log n$ and $\log (n+1)$ is insignificant. There's no reason for worrying about this difference.} weighted imbalance across every interval of
...
which client an adaptive salesperson should probe next.
...
\sharat{Better title: Almost-Optimal A Priori Tours via Dynamic Programming}

\sharat{Rephrase. Our main focus in this chapter is to obtain almost-optimal a priori tours for StochTSP. Since a priori and a posteriori optimums are the same for deterministic instances of TSP and the latter problem is APX-hard, P vs. NP conjecture rules out polynomial time approximation schemes for the a priori problem. 
}
...
Clairvoyance gap $\ge 10/9$ & Symmetric & Theorem~\ref{thm:sym-clair} \\
One stochastic vertex: gap $\le (2+p)/(2+p^2)$ & Symmetric & Theorem~\ref{thm:general-upper} \\
...
$(1{+}\varepsilon)$-approx.\ for a~priori TSP & Both & Theorem~\ref{thm:directed-apriori-ptas} \\
\bottomrule
\end{tabular}
\end{center}
...
\begin{enumerate}
    ...
    of~$10/9$.  Theorem~\ref{thm:sym-clair} shows that this is exact when
...
This appendix records computational searches that provided early exploratory
guidance for the symmetric clairvoyance gap.  These runs predate the balanced
edge lengths in Chapter~\ref{ch:sym-clair}.  The search
procedures were heuristic and limited in scope, so the table below should be
read only as a record of tested instances, not as proof of tightness.
...
The search did not find a ratio above $11/10$ in these runs.  The listed
instances attaining $11/10$ use the earlier equal-crossing-length three-chain
family with one stochastic vertex of activation probability~$1/2$.  The
balanced
family of Chapter~\ref{ch:sym-clair} has a ratio strictly above~$11/10$ for
every $m\ge3$ and tends to~$10/9$; it was found analytically rather than by
these searches.
