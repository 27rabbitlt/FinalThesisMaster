...
\begin{enumerate}
    ...
    We give a balanced three-chain family with one stochastic vertex of
    ...
    \]
    for every symmetric instance with one stochastic vertex of activation
...
symmetric instances with one stochastic vertex.
...
This chapter proves that the clairvoyance gap for symmetric \StochTSP{} is at
least~$10/9$.  We construct a balanced three-chain family with one stochastic
vertex of activation probability~$1/2$ for which
\[
  \frac{\optadapt}{\optpost}
  =\frac{10m+2}{9m+2}
  \longrightarrow\frac{10}{9}.
\]
The lower bound arises because the active and inactive realizations have
different optimal order, while an adaptive policy must begin
probing before knowing which order is required.

As a complementary result, we prove that every symmetric instance with one
stochastic vertex of activation probability~$p\in[0,1]$ satisfies
\[
  \frac{\optadapt}{\optpost}\le\frac{2+p}{2+p^2}.
\]
The right-hand side is maximized at $p=\sqrt6-2$, giving the uniform
one-stochastic-vertex bound
\[
...
clairvoyance gap for symmetric \StochTSP{} is at least~$10/9$.
\end{theorem}
...
deterministic non-root vertices.
...
one-stochastic-vertex class.

\begin{remark}[Tightness parameters of the construction]
The three-chain family asymptotically attains the equality conditions in the
proof of \cref{thm:general-upper} at $p=1/2$.  Indeed,
\[
  \ell_0=L_0=16m+4,
  \qquad
  \Delta=L_1-L_0=4m.
...
\begin{enumerate}
    ...
    there is exactly one stochastic vertex, but a matching upper bound for
