...
\subsection{Maximal inequalities and symmetrization}
...
We also use the standard symmetrization inequality.  Let
$W_1,\ldots,W_m$ be independent random variables, let $\mathcal F$ be a
finite family of tuples $f=(f_1,\ldots,f_m)$ of real-valued functions, and
let $\varepsilon_1,\ldots,\varepsilon_m$ be independent Rademacher signs,
independent of the~$W_i$.  Assume that all random variables in the display
below are integrable.  Then
\begin{equation}\label{eq:symmetrization}
\begin{split}
    &\E\left[
      \sup_{f\in\mathcal F}
      \left|\sum_{i=1}^m
      \bigl(f_i(W_i)-\E[f_i(W_i)]\bigr)\right|
      \right]\\
    &\qquad\le
      2\E\left[
      \sup_{f\in\mathcal F}
      \left|\sum_{i=1}^m\varepsilon_i f_i(W_i)\right|
      \right].
\end{split}
\end{equation}
...
The difficulty is not merely to find inexpensive attachments.  Once the
active clients are realized, their directed attachments need not be balanced,
and correcting this imbalance after seeing the realization would violate
adaptivity.  The proof therefore balances the attachments in expectation
before any client is probed and treats the realized imbalance as a centered
random process.

We first construct a low-cost fractional circulation on a sampled backbone.
We then publicly sample one backbone chord for each outside client and round
an auxiliary flow whose divergence nearly cancels the weighted chord
divergence on every cyclic interval.  During execution, only the independent
activation indicators fluctuate; a one-dimensional maximal inequality bounds
their maximum interval imbalance, and an active-first stack walk implements
the chords adaptively.  This gives a low-mass bound of
$O(\log n+\sqrt\mu)\optpost$, where~$\mu$ is the expected number of active
clients.  Finally, separating clients at activation probability $n^{-1/3}$
balances this policy against a fixed tour for the high-probability clients and
proves \cref{thm:asymmetric-clairvoyance-upper}.
...
small total cost.  The construction cuts an optimal tour through two
independent realizations at the backbone vertices.  Its only loss is the
largest number of outside vertices in one resulting segment; this longest run
has expectation $O(\log n)$.
...
Let $L:=\lceil\log_2(\max\{n,2\})\rceil$.  The tail-sum formula
...
\begin{lemma}[Backbone certificate]
...
\begin{remark}[An offline variant]
By boosting the backbone and rounding each outside client to one chord with
small interval discrepancy, Hoffman's circulation theorem~\cite{Hoffman1960}
yields, after the active set is known, an Eulerian circulation whose used
edges have multiplicity $O(\log n)$.  This construction depends on the entire
activation realization and hence gives no adaptive policy; we therefore omit
its proof and turn directly to the causal rounding argument.
\end{remark}
...
The adaptive construction must choose its endpoint assignments and auxiliary
flow before it learns which clients are active.  For an induced subinstance
on $W\subseteq V^\circ$, write
\[
    \optpost(W):=\E[\opt(A\cap W)]
\]
and let $\optadapt(W)$ denote its adaptive optimum.  We turn the backbone
certificate of \cref{lem:ordinary-backbone-certificate} into such a policy.
The auxiliary flow cancels the \emph{expected} divergence of the client
chords on every cyclic interval up to one unit; the realization then
contributes only a centered sum of independent Bernoulli variables.
...
\[
    \chi_I(a,b):=\1_{\{a\in I\}}-\1_{\{b\in I\}}.
\]
Every linear interval is the difference of two prefixes of the backbone
order, and every cyclic interval is a linear interval or its complement.
Consequently, ordinary maximal inequalities for partial sums control all
cyclic-interval imbalances without a union bound.  We use the following two
forms: the first for the activation variables and the second for the public
endpoint samples.
...
ordered arbitrarily.  The $L^2$ maximal inequality~\cref{eq:l2-maximal} gives
\[
...
For part~\textup{(ii)}, first consider the random tails.  The symmetrization
inequality~\cref{eq:symmetrization}, with independent Rademacher
signs~$\varepsilon_i$, gives
\begin{align*}
...
\end{align*}
For the last inequality, condition on all tails, order them, and apply
\cref{eq:l2-maximal} to the partial sums of
...
Endpoint sampling fixes the weighted divergence~$b(P)$, while the fractional
backbone flow has divergence~$-\bar b$.  The discrepancy~$b(P)-\bar b$ is
small on every cyclic interval in expectation.  The next step corrects this
discrepancy along the backbone cycle and rounds the corrected flow to an
integer one.  The resulting auxiliary flow depends only on the public
endpoint outcome~$P$, not on the activation set.
...
After the activations are realized, the active client chords and the public
...
We can now combine the public preprocessing with the adaptive traversal.  The
only realization-dependent loss is the interval imbalance of the centered
activation indicators, whose expected maximum is $O(\sqrt\mu)$.
...
backbone certificate, sample the public endpoint pairs~$P$, and
...
$O(\log n+\sqrt{n\tau})\optpost$.  Clients above the threshold admit a fixed
tour with factor $O(1/\tau)$.  The following two lemmas justify combining the
subinstances and establish the fixed-tour bound; we then choose~$\tau$ to
...
The deterministic auxiliary-flow rounding leaves only the centered
activation process at runtime, so the maximal inequality incurs no
union-bound logarithm.  Improving the exponent within this framework would
therefore require a better treatment of either the $1/\tau$ fixed-tour term
or the $\sqrt{n\tau}$ low-mass term.
