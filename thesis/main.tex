...
with probability~$p_v$, and put $R:=S\cup\{r\}$. \sharat{Would be ideal to avoid having both $R$ and $S$. Just use one of them. Preferable $S$ should already contain the root.} Let~$T_R$ be an optimal
tour on~$R$.  For every vertex $u\notin R$, choose a nearest backbone vertex
$\mu_S(u)\in R$ and put $D_S(u):=d(u,\mu_S(u))$. \sharat{Bad choice to use $\mu$ for nearest vertex. Use $\pi$ or $\sigma$.} Joining~$u$ to
$\mu_S(u)$ by a doubled spoke \sharat{by a pair of parallel edges for every $u \notin S$ gives an Eulerian spanning graph. This Eulerian graph can be used to obtain a master tour ... } walking around the resulting structure
...
of leaving the backbone and returning to it is at most~$2\optpost$. \sharat{Use $\cdot$ when separating a number $2$ from $\optpost$ for clarity. Do this throughout the thesis.}
