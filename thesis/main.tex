...
of $5/4$ for the adaptivity gap, i.e., between a~priori and adaptive \StochTSP{}.
...
\]
viewed as an ordered path.  The policy first probes $s_1$, then $s_n$, and
...
We next bound its cost for an arbitrary realization~$A$.  The movement
...
for separating adaptive and a~priori solutions.  Its semi-active vertices
activate randomly, and deleting the inactive ones creates shortcut edges
that a fixed master tour must pay for.  Although the master tour knows the metric, it must
...
We now amplify this mechanism by moving the randomness from the activation of
the semi-active vertices to the metric.  The metric will contain a sparse
random collection of cheap edges.  A fixed a~priori order typically obtains little benefit from
them, because the consecutive active vertices in its shortcut are determined by
independent activation randomness.  An adaptive policy can instead probe all
cheap neighbors of its current location before probing a vertex at distance~$2$.
...
We first specify the random family used in the probabilistic construction.
Edges of an Erdős--Rényi graph receive distance~$1$ and all remaining pairs
receive distance~$2$; the activation probability is chosen so that many
vertices are active while cheap active continuations remain sparse.

All logarithms in this section are natural \sharat{all logs are to the base $\mathrm{e}$}.  Fix a sufficiently large
integer~$n$, let $V:=\{r\}\cup[n]$, and set
\[
    p:=\frac{1}{(\log n)^4},
    \qquad
    \theta:=\frac{1}{\log n},
...
has nearly the same large expected cost.  The argument first expands the cost
of one fixed order into weighted edge contributions and then combines
concentration with a union bound over all orders.
...
in~\cref{eq:random-metric-expansion}.  Since $D(e)=2-X_e$ and the
...
\[
    ...
    \right).
\]
Using \cref{lem:random-metric-coefficients} and $W\ge np$, we obtain
\begin{align*}
    \P_D\left[
        C_D(T)\le(2-\theta-\varepsilon)W
    \right]
...
There are at most $n!$ master tours with the root fixed.  A union bound
...
Suppose the salesperson is at~$u$ and the set of unprobed vertices
is~$S$.  If $S=\varnothing$, return to~$r$.  If $u$ has a cheap neighbor
in~$S$, probe the \sharat{vertex with the smallest label. mention early on that the vertices are labeled $1,\ldots,n$.} least-labeled such vertex; otherwise probe the least-labeled
vertex in~$S$.  Thus all unprobed cheap neighbors of the current location
are probed before any vertex at distance~$2$.
...
At the initial state $u=r$, the edges from the root to all unprobed vertices
are likewise unexposed in this analysis.

The next traveled edge has length~$2$ exactly when none of the $s$ remaining
active vertices is a cheap neighbor of~$u$.  Its conditional expected length
is therefore
\[
    1+(1-\theta)^s.
\]
...
\begin{proof}
By~\cref{eq:random-metric-growth}, $\delta_n=o(1)$.  The nonnegative random
...
This chapter gives a sharp result for symmetric \StochTSP{} instances with
one stochastic vertex.  We first construct a balanced three-chain family
with one vertex active with probability~$1/2$.  Its clairvoyance gap is
\[
...
If $N_\times=5$, let $N_A$ be the number of crossings incident to $A$ and $N_s$ the
...
the entire active expanded walk, so all probes of its deterministic vertices must
...
vertex $s$, none of the vertices of the preceding chain could have been probed
...
\begin{align}
    F_A^-(X)
    &\le2\mu^-(X)+C_0\iota(X)\log(n+1),
    \label{eq:cut-concentration-in}\\
    \mu^+(X)
    &\le2F_A^+(X)+C_0\iota(X)\log(n+1).
...
positions.  Set $t=C_0k\log(n+1)$, choose $C_0$ sufficiently large, and take
...
\[
    \bigl|
       \mu^+(X)-\mu^-(X)+y^+(X)-y^-(X)
    \bigr|
    \le C_\Delta\iota(X)\log(n+1).
\]
In particular,
\begin{equation}\label{eq:mean-cut-comparison}
    \mu^-(X)
    \le\mu^+(X)+y^+(X)+C_\Delta\iota(X)\log(n+1).
\end{equation}
Combining
...
This chapter studies how to compute a nearly optimal a~priori solution.  An
a~priori solution fixes a tour before the active vertices are known, and its
realization is obtained by shortcutting past the inactive vertices.  Finding
the best tour directly is difficult because vertices that are far apart in
its order may become consecutive after shortcutting.

The main result of the chapter is a deterministic
...
The main difficulty is that shortcutting creates dependencies between vertices
that are far apart in the order.  In \Cref{sec:dp-pairwise-objective}, we
...
After truncating the pairwise objective, we still need to optimize the remaining 
pairs. To achieve this efficiently, we partition vertices into blocks and optimize 
the order blocks instead of the order of vertices. We call the new optimization objective
\emph{block objective}.
We again prove that block objective is a good approximation, which is explained in 
details in \cref{lem:block-comparison}.

To obtain such a partition, we keep vertices with large activation
probability in singleton blocks and group the remaining vertices into blocks
of small total activation probability.  For a pair whose endpoints lie in
different blocks, $F_\eta$ retains the inactivity probabilities of the
complete intervening blocks but omits the factors contributed inside the two
endpoint blocks.  Omitting these factors gives an upper bound, while the small
mass of each nonsingleton block controls the resulting overestimate.  The
block form also allows the dynamic program in the next section to evaluate a
new block using only a bounded suffix of the preceding blocks.

Fix $0<\delta<1/4$ and recall that $q_v=1-p_v$.  A non-root vertex is \emph{heavy} if
$p_v>\delta/2$ and \emph{light} otherwise.  For a set of non-root vertices~$B$, write
\[
...
\begin{enumerate}[label=(\alph*)]
    \item a singleton containing one heavy vertex, or
    \item a nonempty set of light vertices with total mass at most~$\delta$.
\end{enumerate}
...
\end{equation}
where the minimum is over all orders~$\tau$ of~$B$.  For a heavy singleton
and for the root block, set $I=0$.  The values $I(B)$, together with a
...
\cref{eq:internal-block-cost}, and concatenate the block orders.  Denote the
resulting a~priori tour by~$T_{\mathcal B}$.  Put
$B_0=B_{m+1}=\{r\}$ and recall that $p_r=1$.  Define
\[
    C(B_i,B_j)
    :=
...
\begin{enumerate}[label=(\roman*)]
    ...
    partition $\mathcal B_T$ into consecutive sets of~$T$ such that
...
In the tour $T_{\mathcal B}$, their exact coefficient is
\[
    p_xp_y\,
    q(B_i^{>x})\,
...
\cref{eq:block-objective}.  Dropping the two endpoint factors can only
...
retained pairs proves~\cref{eq:block-objective-dominates-local}.

For~(ii), keep every heavy vertex as a singleton.  In each maximal consecutive
run of light vertices in~$T$, scan from right to left.  Close a block as soon
...
vertex has probability at most~$\delta/2$.  The only possible block of mass
below~$\delta/2$ is the leftmost block of the run.  The resulting partition
$\mathcal B_T$ is therefore admissible.
...
a root block, the corresponding factor is~$1$.  For a light block~$B$,
\[
    q(B')\ge1-\sum_{v\in B'}p_v\ge1-\delta
    \qquad\text{for every }B'\subseteq B.
\]
Thus every cross-block coefficient in $F_\eta(\mathcal B_T)$ is at most
$(1-\delta)^{-2}$ times the corresponding exact coefficient in~$T$.

Finally, let $B$ be a light block, and retain the order induced by~$T$ on~$B$.
If $W_B(T)$ denotes the exact contribution of pairs with both endpoints
in~$B$, then
\[
    I(B)
    \le
...
We now combine the truncation lemma, the block comparison, and the exact
dynamic program.  Choosing their parameters as functions of the target error
$\varepsilon$ makes the accumulated multiplicative loss at most
$1+\varepsilon$ while keeping the running-time base dependent only on
$\varepsilon$.
...
\cref{lem:block-comparison}(ii), there is an admissible ordered partition
...
$\widehat T:=T_{\widehat{\mathcal B}}$.  \Cref{lem:block-comparison}(i)
gives
\[
    \cost^{\loc}_{\eta}(\widehat T)
    \le F_\eta(\widehat{\mathcal B})
    \le F_\eta(\mathcal B^*)
    \le(1-\delta)^{-2}\optprior.
\]
Applying \cref{lem:long-gap-truncation},
...
\]
Substituting these bounds into~\cref{eq:final-approximation-chain} yields
\[
    \cost(\widehat T)
    \le
