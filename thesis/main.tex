...
$O(\log^2(n+1)+\sqrt\mu)\optpost$, where~$\mu$ is the probability mass.
...
We first construct a low-cost fractional circulation attaching the clients
outside a sampled backbone to a tour on that backbone.  We then round the
client attachments and the auxiliary backbone flow jointly, before any
...
Call an unsampled client an \emph{outside client}.  We require every outside
client~$v$ to have fractional in-degree and out-degree~$p_v$, and forbid flow
between two outside clients.
...
of~$R:=S\cup\{r\}$ produces directed segments
\[
...
Let $L:=\lceil\log_2(n+1)\rceil$.  The tail-sum formula
...
\[
    ...
    \qquad
    \E_S[\cost(z^S)]
    \le C_{\mathrm{back}}\log(n+1)\,\optpost(W)
\]
...
The endpoint assignments and auxiliary arcs must be chosen before the policy
learns which clients are active.  We obtain both objects in one randomized
...
There is a randomized procedure, independent of the client activations, that
outputs an ordered pair $(a_i,b_i)\in R\times R$ for every $i\in U$ with
$p_i>0$ and a nonnegative integer vector $N=(N_{ab})_{a,b\in R}$.  Every
outcome satisfies
\[
...
\begin{equation}
    \left|
     \sum_{i\in U:p_i>0}p_i\chi_I(a_i,b_i)
       +\sum_{a,b\in R}N_{ab}\chi_I(a,b)
     \right|
      \le C_{\mathrm{round}}\log^2(n+1),
...
unique client variable for each positive-probability client and round
...
We preserve every outside-client equation
\begin{equation}\label{eq:joint-rounding-client-equation}
...
number of client equations containing a live variable.  Such an equation
cannot contain exactly one live variable:
all its other variables are in~$\{0,p_i\}$, so
\cref{eq:joint-rounding-client-equation} would force the remaining variable
...
For $p_i>0$, \cref{eq:joint-rounding-client-equation} now implies that exactly
one variable~$\xi_{i,a_ib_i}$ equals~$p_i$.  Put $N_{ab}:=\eta_{ab}$.  The
...
subsequently change.  A client variable changes by at most~$p_i\le1$, and a
backbone variable changes by at most one because its bounds are consecutive
...
gives final discrepancy $O((h+1)L)=O(\log^2(n+1))$ for every cyclic interval,
which proves~\cref{eq:joint-rounding-interval}.
\end{proof}

Since $R$ is finite and $N_{ab}\le\lceil y_{ab}\rceil$ for every pair, every
rounding outcome produces a finite multiset~$N$.  It need not be a
circulation by itself; only its combined imbalance with the selected client
chords will be used.

\begin{remark}[Why an offline completion is insufficient]
If the active set were known in advance, one could repair the imbalance of
the realized chord multiset by adding backbone flow and then extract an Euler
tour.  Since this repair depends on the full activation realization, it does
not define an adaptive policy.  The next section instead traverses the fixed
rounded objects causally.
\end{remark}

\section{An adaptive circulation policy}
\label{sec:adaptive-circulation}

The fractional construction and randomized rounding are now complete.  For
the moment, fix a backbone~$S$ and one outcome~$\rho$ of the rounding.  The
resulting pairs~$(a_i,b_i)$ and multiset~$N$ depend only on this preprocessing,
not on the actual activation set.  For a client with $p_i=0$, set
$(a_i,b_i):=(r,r)$; such a client is certainly inactive, but this convention
ensures that it is included in the probing lists below.  We now turn these
fixed rounded objects into a causal traversal.  Once the activation set is
realized, the only additional interval imbalance is a centered sum of
independent Bernoulli variables.

\subsection{Adaptive traversal}
\label{sec:adaptive-stack-traversal}

After the activations are realized, the active client chords and the fixed
auxiliary arcs form a multiset of chords on the backbone cycle.  Their
interval imbalance determines how often the policy must use the cycle edges.
The following traversal rule realizes this relationship directly.
...
For an activation set~$A$, let
\[
    F(A):=N\uplus\biguplus_{i\in A\cap U}\{a_i\to b_i\},
\]
where $N_{ab}$ denotes the multiplicity of the auxiliary arc~$a\to b$.

\begin{lemma}
\label{lem:adaptive-virtual-execution}
The rounded pairs and auxiliary multiset define a causal strategy whose
actual cost on every realization~$A$ is at most
\begin{align}
    &\sum_{a,b\in R}N_{ab}d(a,b)
    +\sum_{i\in A\cap U}\bigl(d(a_i,i)+d(i,b_i)\bigr)\notag\\
    &\hspace{32mm}
    +(\Lambda(F(A))+2)\cost(T_R).
    \label{eq:adaptive-virtual-cost}
\end{align}
Every client is probed, and every active client is visited immediately after
it is probed.  Consequently, there is an adaptive policy in the formal sense
of \cref{sec:stoch-tsp-benchmarks} whose expected cost is no larger than that
of this strategy.
\end{lemma}
...
Every choice uses only fixed preprocessing data and outcomes already
revealed.
...
Inactive probes cause no movement.  Thus, inductively, the actual cost paid
plus the pending length is at most the virtual length charged.

After the last chord of~$F(A)$ is used, some inactive clients may remain
...
The strategy is therefore nonanticipating.  The Bellman--Markovization
argument in \cref{sec:stoch-tsp-benchmarks} yields a formal adaptive policy
of no larger expected cost.
\end{proof}

\subsection{The low-mass policy}
\label{sec:low-mass-adaptive-policy}

We can now combine the activation-independent randomized preprocessing with
the adaptive traversal.  Every rounding outcome contributes only
$O(\log^2 n)$ interval discrepancy, and the remaining realization-dependent
loss is the centered activation imbalance, whose expected maximum is
$O(\sqrt\mu)$.
...
\[
    \optadapt(W)
    \le C_3\bigl(\log^2(n+1)+\sqrt\mu\bigr)\optpost(W),
\]
where $C_3$ is universal.
\end{proposition}

\begin{proof}
Put $B:=\optpost(W)$.  If $B=0$, use
\cref{lem:zero-posterior-policy}.  Otherwise, let~$S\subseteq W$ be an
auxiliary sample with the activation probabilities, independent of the
actual activation set~$A$.  For every realization of~$S$ in its support,
take the circulation~$z^S$ and an optimal backbone tour~$T_R^S$, and apply
\cref{lem:joint-circulation-rounding} with independent random tape~$\rho$.
Thus $\cost(T_R^S)=\opt(S)$.  Fix~$(S,\rho)$ temporarily.  For every cyclic
interval~$I$, the realized imbalance decomposes as
\begin{align*}
...
\]
Order the clients once by the positions of their tails~$a_i$, breaking ties
in an arbitrary fixed way.  The first process on the right, evaluated at
prefix boundaries, is a subsequence of the partial sums in this order.  Hence
...
\[
    ...
    \le2\sqrt\mu.
\]
Ordering the clients by their heads~$b_i$, again with fixed tie-breaking,
gives the same bound for the second process, and therefore
\[
...
\begin{equation}\label{eq:adaptive-active-imbalance}
    \E_A[\Lambda(F(A))]
    \le C_{\mathrm{round}}\log^2(n+1)+8\sqrt\mu.
\end{equation}

Take activation expectation in the causal-strategy bound
\cref{eq:adaptive-virtual-cost}.  By
\cref{lem:adaptive-virtual-execution}, there is a formal adaptive
policy~$\pi_{S,\rho}$ with no larger expected cost.  Put
\[
    D:=C_{\mathrm{round}}\log^2(n+1)+8\sqrt\mu+2.
\]
Using \cref{eq:adaptive-active-imbalance} gives
\begin{align*}
    \cost(\pi_{S,\rho})
    &\le
      \sum_{a,b\in R}N_{ab}d(a,b)
    ...
    &\qquad +D\cost(T_R^S).
\end{align*}
The quantity~$D$ is independent of both preprocessing choices.  Averaging
first over~$\rho$ and using \cref{eq:joint-rounding-cost}, then averaging
over~$S$ and using \cref{lem:average-backbone-cost}, gives
\begin{align*}
    \E_{S,\rho}[\cost(\pi_{S,\rho})]
    &\le \E_S[\cost(z^S)]+D\E_S[\opt(S)]\\
    &\le \bigl(C_{\mathrm{back}}\log(n+1)+D\bigr)B\\
    &\le C_3\bigl(\log^2(n+1)+\sqrt\mu\bigr)B
\end{align*}
...
probed produces a deterministic adaptive policy and proves the claim.
\end{proof}

\section{Threshold optimization and the clairvoyance-gap bound}
\label{sec:asymmetric-threshold-optimization}

For a probability threshold~$\tau$, clients below the threshold have total
activation mass at most $n\tau$, so the preceding policy costs
$O(\log^2(n+1)+\sqrt{n\tau})\optpost$.  Clients above the threshold admit a
...
The case $n=0$ is trivial, so assume $n\ge1$.
...
\]
By \cref{lem:high-probability-fixed-tour}, the high-probability clients have
...
\]
Thus \cref{prop:adaptive-low-mass} supplies an adaptive policy~$\pi_W$ with
\[
    \cost(\pi_W)
    \le C_3\bigl(\log^2(n+1)+n^{1/3}\bigr)\optpost(W).
\]
Let~$\pi$ execute~$\pi_H$ and~$\pi_W$ successively, returning to~$r$
...
\begin{align*}
    ...
    &\le
      \left(
        ...
        +C_3\bigl(\log^2(n+1)+n^{1/3}\bigr)
      \right)\optpost\\
    &\le C^\star n^{1/3}\optpost
\end{align*}
for a universal constant~$C^\star$, where we used
$\lceil1/\tau\rceil\le2n^{1/3}$ and
$\log^2(n+1)=O(n^{1/3})$.  Enlarging the constant~$C$ in the theorem to
$C^\star$ proves the claim.
\end{proof}

The joint rounding separates the interval imbalance into a pathwise
$O(\log^2 n)$ discrepancy and a centered activation process of expected size
$O(\sqrt{n\tau})$.  Thus the exponent is determined by balancing the
$1/\tau$ fixed-tour term against the latter activation term; the logarithmic
rounding loss is lower order at the chosen threshold.
